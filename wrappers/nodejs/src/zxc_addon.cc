...
// True when the optional output cap at info[i] (validated in JS) is below size.
static bool ExceedsOutputCap(const Napi::CallbackInfo& info, size_t i, uint64_t size) {
    return info.Length() > i && info[i].IsNumber() &&
           size > static_cast<uint64_t>(info[i].As<Napi::Number>().Int64Value());
}
...
static Napi::Value Decompress(const Napi::CallbackInfo& info) {
    ...
    if (ExceedsOutputCap(info, 5, decompress_size)) {
        return ThrowZxcError(env, ZXC_ERROR_DST_TOO_SMALL);
    }
...
class DctxWrap : public Napi::ObjectWrap<DctxWrap> {
   ...
   private:
    ...
    Napi::Value Decompress(const Napi::CallbackInfo& info) {
        ...
        Napi::Buffer<uint8_t> src = info[0].As<Napi::Buffer<uint8_t>>();
        const uint64_t declared = zxc_get_decompressed_size(src.Data(), src.Length());
        if (ExceedsOutputCap(info, 1, declared)) return ThrowZxcError(env, ZXC_ERROR_DST_TOO_SMALL);
...
static constexpr double kMaxSafeInteger =
    static_cast<double>((uint64_t{1} << std::numeric_limits<double>::digits) - 1);

// Reads a 64-bit block count or index from a JS number. Those are doubles, so a
// value above kMaxSafeInteger would arrive already rounded.
static bool ExactUint64FromJs(Napi::Env env, Napi::Value v, const char* what, uint64_t* out) {
    const double d = v.As<Napi::Number>().DoubleValue();
    // Rejects NaN too: every comparison against it is false.
    if (!(d >= 0.0) || !(d <= kMaxSafeInteger) || d != std::trunc(d)) {
        std::string msg = std::string(what) + " must be an integer in [0, 2^53)";
...
class SeekableWrap : public Napi::ObjectWrap<SeekableWrap> {
   ...
   private:
    ...
    Napi::Value NumBlocks(const Napi::CallbackInfo& info) {
        ...
        const uint64_t n = zxc_seekable_get_num_blocks(s_);
        return Napi::Number::New(info.Env(), static_cast<double>(n));
    }
    ...
    Napi::Value BlockCompressedSize(const Napi::CallbackInfo& info) {
        ...
        uint64_t idx = 0;
        if (!ExactUint64FromJs(env, info[0], "block index", &idx)) return env.Undefined();
...
static Napi::Value SeekTableSize(const Napi::CallbackInfo& info) {
    ...
    uint64_t n = 0;
    if (!ExactUint64FromJs(env, info[0], "numBlocks", &n)) return env.Undefined();
...
static Napi::Object Init(Napi::Env env, Napi::Object exports) {
    ...
    exports.Set("ERROR_DICT_UNSUPPORTED", Napi::Number::New(env, ZXC_ERROR_DICT_UNSUPPORTED));
