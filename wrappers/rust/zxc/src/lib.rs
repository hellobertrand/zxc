...
/// (e.g. `"0.11.0"`). Useful for verifying that the dynamically-linked
...
/// threaded; for parallel file-to-file compression, use [`compress_file`].
///
/// # Example
///
/// ```rust,no_run
...
mod pstream_tests {
    ...
    fn pstream_roundtrip(data: &[u8], copts: Option<&CompressOptions>, dopts: Option<&DecompressOptions>) -> Vec<u8> {
        ...
        while cursor < data.len() {
            loop {
                ...
                if p.pending == 0 && (p.consumed > 0 || cursor == data.len()) { break; }
            }
        }
        loop {
    ...
    fn pstream_with_checksum() {
        ...
        let copts = CompressOptions { level: Level::Default, checksum: true, seekable: false };
        let dopts = DecompressOptions { verify_checksum: true };
