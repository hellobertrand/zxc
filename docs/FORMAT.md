...
- The checksum input is the compressed payload bytes only, NOT the
  block header.
- The algorithm identifier 0 denotes fold32(rapidhash(payload)),
  where rapidhash is RapidHash v3 {{RAPIDHASH}} applied with its
  default secret and a seed of 0, and fold32 is:
...
2. Verify that the identifier of the supplied dictionary equals the
   ...
   ({{shared-huffman-literal-section}}). A dictionary supplied
   without a shared table, which an archive MUST NOT use for
   enc_lit = 3 sections, is identified by the content hash alone:
   fold32(rapidhash(content)).
...
: A deterministic 32-bit identifier that binds the (content, table)
  pair. Each byte is hashed exactly once: the folded hash of the
  Dictionary Content seeds the hash of the 128-byte Shared Literal
  Huffman Table, with rapidhash and fold32 as defined in
  {{per-block-checksum}}:

~~~
base    = fold32(rapidhash(content))
dict_id = fold32(rapidhash(table_128_bytes, seed = base))
~~~

: The seed is that folded 32-bit value, zero-extended to the 64-bit
  seed parameter of RapidHash. Seeding with the unfolded 64-bit hash
  yields a different identifier, so an implementation that does so
  will have its dictionaries rejected. It MUST equal the dict_id
