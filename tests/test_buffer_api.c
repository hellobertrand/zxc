...
// Level 3 encoder choice: fast_encode=1 selects the classic GLO encoder,
// fast_encode=0 (default) the GUL encoder. Both must roundtrip; the first
// data block's type must reflect the choice on GUL-friendly input.
int test_roundtrip_l3_fast_encode(void) {
    printf("=== TEST: Level 3 fast_encode (GLO) vs default (GUL) ===\n");
    const size_t n = 256 * 1024;
    uint8_t* src = malloc(n);
    uint8_t* dec = malloc(n);
    const size_t cap = (size_t)zxc_compress_bound(n);
    uint8_t* c0 = malloc(cap);
    uint8_t* c1 = malloc(cap);
    if (!src || !dec || !c0 || !c1) return 0;
    gen_gul_data(src, n);

    int ok = 0;
    zxc_compress_opts_t o_gul = {.level = 3};
    zxc_compress_opts_t o_glo = {.level = 3, .fast_encode = 1};
    const int64_t s0 = zxc_compress(src, n, c0, cap, &o_gul);
    const int64_t s1 = zxc_compress(src, n, c1, cap, &o_glo);
    if (s0 <= 0 || s1 <= 0) {
        printf("Failed: compression (%lld / %lld)\n", (long long)s0, (long long)s1);
        goto done;
    }
    /* First data block type: offset 16 in the archive (after the file header).
     * GUL = 3 on this record-pool input; fast_encode must NOT emit GUL. */
    if (c0[16] != 3) {
        printf("Failed: default level 3 should emit a GUL block (got type %u)\n", c0[16]);
        goto done;
    }
    if (c1[16] == 3) {
        printf("Failed: fast_encode should not emit GUL blocks (got type %u)\n", c1[16]);
        goto done;
    }
    zxc_decompress_opts_t d = {0};
    if (zxc_decompress(c0, (size_t)s0, dec, n, &d) != (int64_t)n || memcmp(dec, src, n) != 0) {
        printf("Failed: GUL roundtrip\n");
        goto done;
    }
    if (zxc_decompress(c1, (size_t)s1, dec, n, &d) != (int64_t)n || memcmp(dec, src, n) != 0) {
        printf("Failed: fast_encode roundtrip\n");
        goto done;
    }
    printf("PASS\n\n");
    ok = 1;
