...
/**
 * @brief Length of the common prefix of two 32-byte regions (0..32).
 *
...
static ZXC_ALWAYS_INLINE uint32_t zxc_gul_lcp32(const uint8_t* RESTRICT a,
    ...
    if (m0 != ~(uint64_t)0) return (uint32_t)(zxc_ctz64(~m0) >> 2);
...
static ZXC_ALWAYS_INLINE int zxc_encode_block_gul_impl(
    ...
    if (UNLIKELY(ctx->gul_htab == NULL)) return ZXC_ERROR_NULL_INPUT;

    uint16_t* const htab = ctx->gul_htab;
    uint8_t* const hidx = ctx->gul_hidx;
    ...
    const size_t limit = end - ZXC_GUL_TAIL_MIN; /* no match may end past this */

    size_t pos = dict_sz; /* scan cursor (concat coordinates) */
    size_t lit = dict_sz; /* start of the pending literal run */
    size_t hpos = 0;      /* insertion cursor (covers a dict prefix lazily) */
    size_t so = 0;        /* token-stream bytes emitted */
    size_t lo = 0;        /* literal bytes emitted */
    uint32_t n_seq = 0;
    ...
    const size_t tail_len = end - lit; /* >= ZXC_GUL_TAIL_MIN by the limit */
    const size_t fixed =
        ZXC_GUL_HEADER_BINARY_SIZE + ZXC_GUL_SECTIONS * ZXC_SECTION_DESC_BINARY_SIZE;
    const uint64_t payload = (uint64_t)fixed + lo + so + tail_len;
